...
- **Last updated:** 2026-09-15T15:32:30+08:00.
- **Last reviewed Git revision:** `6795b61326f1c7d4b573592149b5a27b9c9e506c` on branch `RF-11-Params`.
...
- **Current source-only worktree:** The eight approved RF source/config files plus five synthetic pytest files implement the D-009/D-010 guardrails. Scenario rows now require explicit wind-matched state-transition evidence; manifests are versioned and recursively bound; split identities match bidirectionally; all four roles require both labels; calibration-method selection requires grouped train/validation evidence; fixed numeric class-cost ratios fail closed; and model/study/summary publication is staged as one immutable version directory. Observation sampling is unset and artifact generation remains blocked until the owner approves a population-preserving policy. No dataset, model, raster, simulation, result, or manuscript operation was performed. Full pytest execution is pending because pytest is absent from the checked Python environments.
...
2. `NEEDS VERIFICATION` — Identify approved state-at-`t`/state-at-`t+1` evidence before generating real transition rows. Current defaults intentionally leave those paths unset and artifact writes disabled.
...
5. `NEEDS VERIFICATION` — Approve the Set C observation-sampling policy before dataset generation. The source currently permits only explicit `all_eligible`; uncorrected case-control sampling is rejected because it would alter the prevalence used to interpret calibrated ignition probabilities.
6. `WARNING` — Phase 4 source guardrails are present only in the working tree and have not been exercised against real datasets, models, or rasters. Artifact compliance and measured impact remain unverified.
7. `WARNING` — Pytest is not installed in the checked Python environments. AST/YAML validation and direct synthetic contract smoke checks pass, but the five pytest files remain unexecuted.
...
No RF experiment result is established for the Phase 4 worktree. On 2026-09-15, read-only AST parsing passed for seven changed Python sources and five tests; YAML/import checks passed; and direct synthetic smoke checks passed for schema/value validation, transition timing, explicit wind/outcome binding, exact bidirectional split-manifest membership, per-role class coverage, group isolation, grouped non-test calibration-selection provenance, calibration-only reporting-threshold provenance, reordered positive classes, unchanged stochastic CA probability, and building-only spatial metrics. Pytest collection/execution remains blocked because pytest is absent. These checks did not train, generate data, simulate, deserialize a model, or inspect raster artifacts.
